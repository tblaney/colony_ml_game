    ...
    void PathCallback()
    {
        ...
        } else
        {
            ...
            if (distance < _interactionDistance)
            {
                ...
                _targetNode.Damage(_controller.GetBot().GetDamage(HabBot.State.CollectMinerals));
                Invoke("PathCallback", _controller.GetBot().GetInteractTime(HabBot.State.CollectMinerals));
                _interacting = true;
