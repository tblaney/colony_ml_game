...
Shader "ToonTerrain" {
    Properties {
        ...
        _TopSpread("TopSpread", Range(0,1)) = 0.8
    ...
    CGINCLUDE
        ...
        void surf(Input IN, inout SurfaceOutput o)
        {
			...
			bf /= dot(bf, (float3)1);
            //float3 bf = pow(abs(IN.localNormal), 20);
            //bf /= dot(bf, (float3)1);

			// Triplanar mapping
			float2 tx = IN.localCoord.zy * _VerticalTex_ST.x;
            ...
            float3 blendNormal = saturate(pow(IN.localNormal * 1.4, 4));

            float3 sidetexture = cz;
            sidetexture = lerp(sidetexture, cx, blendNormal.x);
            sidetexture = lerp(sidetexture, cy, blendNormal.y);
			

            SplatmapMix(IN, splat_control, weight, mixedDiffuse, o.Normal);
			if (dot(IN.localNormal, fixed3(0, 1, 0)) >= _TopSpread)
			{
